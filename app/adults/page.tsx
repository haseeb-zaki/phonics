...
export default function AdultsPractice() {
  ...
  const loadNewWord = () => {
    if (wordGroup && hasWordsInGroup('adults', wordGroup, wordLength)) {
      const word = getRandomWordFromGroup('adults', wordGroup, wordLength);
      if (word) {
        setCurrentWord(word);
    ...
    } else if (!wordGroup && hasWords('adults', wordLength)) {
      const word = getRandomWord('adults', wordLength);
      if (word) {
        setCurrentWord(word);
        setCurrentLetter(null);
        setFeedback(null);
  ...
  const handleSpellingSubmit = (input: string) => {
    if (mode === 'words') {
      ...
      updateProgress('adults', wordLength, currentWord, isCorrect);
    ...
    <>
      ...
      <main className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4">
        <div className="container mx-auto py-8 max-w-4xl">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 md:p-8 mb-6">
            ...
            {mode === 'words' ? (
              <>
                ...
                <div className="mb-6 bg-indigo-50 dark:bg-indigo-900/20 border-2 border-indigo-200 dark:border-indigo-700 rounded-xl p-4">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-2 border-2 border-indigo-300 dark:border-indigo-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-gray-800 dark:text-white"
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                    ...
                    <button
                      ...
                      className="px-6 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg font-semibold transition-colors"
                    >
                      Load
                    </button>
                  </div>
                ...
                <div className="flex flex-col items-center gap-4 mb-6">
                  {!showManualInput && (
                    <button
                      ...
                      className="px-4 py-2 bg-indigo-400 hover:bg-indigo-500 text-white rounded-lg font-semibold transition-colors"
                    >
                      Need help? Type the word to hear pronunciation
                    </button>
                  )}
                  {showManualInput && (
              ...
              <>
                ...
                {currentLetter && (
                  <>
                    <div className="text-center mb-6">
                      <LetterDisplay letter={currentLetter} userType="adults" />
                    </div>

                    <div className="flex flex-col items-center gap-4 mb-6">
                      <PhonicsSoundPlayer letter={currentLetter} rate={0.95} pitch={1.0} />
                      {!showManualInput && (
                        <button
                          onClick={handleManualInput}
                          className="px-4 py-2 bg-indigo-400 hover:bg-indigo-500 text-white rounded-lg font-semibold transition-colors"
                        >
                          Need help? Type the letter to hear pronunciation
                        </button>
                      )}
                      {showManualInput && (
                        <div className="w-full max-w-md">
                    ...
                    <div className="mb-6 p-4 bg-indigo-50 dark:bg-indigo-900/20 border-2 border-indigo-300 dark:border-indigo-700 rounded-lg">
          ...
          <ProgressTracker userType="adults" wordLength={wordLength} />
        </div>
      </main>
    </>
  );
